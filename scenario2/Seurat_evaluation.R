...
# • counts-layer contains raw counts; leave X (log space) aside
# • data slot (log-normalised) will be generated by Seurat itself
seurat_obj <- as.Seurat(
  adata,
  counts = "counts",     # raw
  data   = NULL          # we'll create fresh log‐norm data
)
rm(adata); gc()

targets <- c("dataset", "location", "patientGroup",
             "protocol", "sanger_type", "sampling_method", "donor")

# -------------- build the summary table ------------------------------------
batch_summary <- seurat_obj@meta.data %>%
  group_by(batch) %>%
  summarise(
    across(all_of(targets), ~ paste(unique(.x), collapse = ", ")),
    .groups = "drop"
  ) %>%
  arrange(batch)

# -------------- view in console (nicely formatted) -------------------------
library(knitr)
kable(batch_summary, caption = "Unique values per batch")
